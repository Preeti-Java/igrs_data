package com.cg.neel.igrs.district;

import javax.persistence.Entity;
import javax.persistence.Table;

import com.cg.neel.igrs.district.common.CommonDeedAccessBean;
import com.cg.neel.igrs.district.help.BookNoAccessBean;
import com.cg.neel.igrs.district.help.DistrictAccessBean;
import com.cg.neel.igrs.district.help.DocumentTypeAccessBean;
import com.cg.neel.igrs.district.help.RegistrationDistrictAccessBean;
import com.cg.neel.igrs.district.help.RevenueDistrictAccessBean;
import com.cg.neel.igrs.district.help.SRLocationAccessBean;
import com.cg.neel.igrs.district.help.SubDistrictAccessBean;
import com.cg.neel.igrs.district.help.YearAccessBean;

@Entity
@Table(name = "KawardhaDeed")
public class KawardhaDeedAccessBean extends CommonDeedAccessBean{

	


	/**
	 * @param sno
	 * @param fileId
	 * @param volumeNo
	 * @param bookNoAccessBean
	 * @param documentNo
	 * @param pageFrom
	 * @param pageTo
	 * @param dateOfRegistration
	 * @param yearAccessBean
	 * @param documentTypeAccessBean
	 * @param registrationDistrictAccessBean
	 * @param srLocationAccessBean
	 * @param revenueDistrictAccessBean
	 * @param districtAccessBean
	 * @param subDistrictAccessBean
	 */
	public static KawardhaDeedAccessBean of(String fileId, String volumeNo, BookNoAccessBean bookNoAccessBean,
			String documentNo, String pageFrom, String pageTo, String dateOfRegistration, YearAccessBean yearAccessBean,
			DocumentTypeAccessBean documentTypeAccessBean,
			RegistrationDistrictAccessBean registrationDistrictAccessBean, SRLocationAccessBean srLocationAccessBean,
			RevenueDistrictAccessBean revenueDistrictAccessBean, DistrictAccessBean districtAccessBean,
			SubDistrictAccessBean subDistrictAccessBean) {
		
		KawardhaDeedAccessBean kawardhaDeedAccessBean = new KawardhaDeedAccessBean();
		kawardhaDeedAccessBean.setFileId(fileId);
		kawardhaDeedAccessBean.setVolumeNo(volumeNo);
		kawardhaDeedAccessBean.setBookNoAccessBean(bookNoAccessBean);
		kawardhaDeedAccessBean.setDocumentNo(documentNo);
		kawardhaDeedAccessBean.setPageFrom(pageFrom);
		kawardhaDeedAccessBean.setPageTo(pageTo);
		kawardhaDeedAccessBean.setDateOfRegistration(dateOfRegistration);
		kawardhaDeedAccessBean.setYearAccessBean(yearAccessBean);
		kawardhaDeedAccessBean.setDocumentTypeAccessBean(documentTypeAccessBean);
		kawardhaDeedAccessBean.setRegistrationDistrictAccessBean(registrationDistrictAccessBean);
		kawardhaDeedAccessBean.setSrLocationAccessBean(srLocationAccessBean);
		kawardhaDeedAccessBean.setRevenueDistrictAccessBean(revenueDistrictAccessBean);
		kawardhaDeedAccessBean.setDistrictAccessBean(districtAccessBean);
		kawardhaDeedAccessBean.setSubDistrictAccessBean(subDistrictAccessBean);
		
		return kawardhaDeedAccessBean;
	}

}
