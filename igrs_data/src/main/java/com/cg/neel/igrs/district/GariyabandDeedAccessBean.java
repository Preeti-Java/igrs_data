package com.cg.neel.igrs.district;

import javax.persistence.Entity;
import javax.persistence.Table;

import com.cg.neel.igrs.district.common.CommonDeedAccessBean;
import com.cg.neel.igrs.district.help.BookNoAccessBean;
import com.cg.neel.igrs.district.help.DistrictAccessBean;
import com.cg.neel.igrs.district.help.DocumentTypeAccessBean;
import com.cg.neel.igrs.district.help.RegistrationDistrictAccessBean;
import com.cg.neel.igrs.district.help.RevenueDistrictAccessBean;
import com.cg.neel.igrs.district.help.SRLocationAccessBean;
import com.cg.neel.igrs.district.help.SubDistrictAccessBean;
import com.cg.neel.igrs.district.help.YearAccessBean;

@Entity
@Table(name = "GariyabandDeed")
public class GariyabandDeedAccessBean extends CommonDeedAccessBean{

	/**
	 * @param sno
	 * @param fileId
	 * @param volumeNo
	 * @param bookNoAccessBean
	 * @param documentNo
	 * @param pageFrom
	 * @param pageTo
	 * @param dateOfRegistration
	 * @param yearAccessBean
	 * @param documentTypeAccessBean
	 * @param registrationDistrictAccessBean
	 * @param srLocationAccessBean
	 * @param revenueDistrictAccessBean
	 * @param districtAccessBean
	 * @param subDistrictAccessBean
	 */
	public static GariyabandDeedAccessBean of(String fileId, String volumeNo, BookNoAccessBean bookNoAccessBean,
			String documentNo, String pageFrom, String pageTo, String dateOfRegistration, YearAccessBean yearAccessBean,
			DocumentTypeAccessBean documentTypeAccessBean,
			RegistrationDistrictAccessBean registrationDistrictAccessBean, SRLocationAccessBean srLocationAccessBean,
			RevenueDistrictAccessBean revenueDistrictAccessBean, DistrictAccessBean districtAccessBean,
			SubDistrictAccessBean subDistrictAccessBean) {
		
		GariyabandDeedAccessBean gariyabandDeedAccessBean = new GariyabandDeedAccessBean();
		gariyabandDeedAccessBean.setFileId(fileId);
		gariyabandDeedAccessBean.setVolumeNo(volumeNo);
		gariyabandDeedAccessBean.setBookNoAccessBean(bookNoAccessBean);
		gariyabandDeedAccessBean.setDocumentNo(documentNo);
		gariyabandDeedAccessBean.setPageFrom(pageFrom);
		gariyabandDeedAccessBean.setPageTo(pageTo);
		gariyabandDeedAccessBean.setDateOfRegistration(dateOfRegistration);
		gariyabandDeedAccessBean.setYearAccessBean(yearAccessBean);
		gariyabandDeedAccessBean.setDocumentTypeAccessBean(documentTypeAccessBean);
		gariyabandDeedAccessBean.setRegistrationDistrictAccessBean(registrationDistrictAccessBean);
		gariyabandDeedAccessBean.setSrLocationAccessBean(srLocationAccessBean);
		gariyabandDeedAccessBean.setRevenueDistrictAccessBean(revenueDistrictAccessBean);
		gariyabandDeedAccessBean.setDistrictAccessBean(districtAccessBean);
		gariyabandDeedAccessBean.setSubDistrictAccessBean(subDistrictAccessBean);
		
		return gariyabandDeedAccessBean;
	}

}
