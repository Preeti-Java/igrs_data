...
@Repository
public interface RaigarhParty2Repository extends MappedTypeParty2Repository<RaigarhParty2AccessBean>{

	@Query(value = "SELECT * FROM Raigarhparty2 p "
			+ "WHERE p.fileid IN (SELECT d.fileid FROM Raigarhdeed d " + "INNER JOIN Raigarhproperty prop "
	...
	List<RaigarhParty2AccessBean> findByMethod(Long year, Long tehsil, Long village,
			Long district);
	
	@Query(value = "SELECT * FROM Raigarhparty2 p "
			+ "WHERE p.fileid IN (SELECT d.fileid FROM Raigarhdeed d " + "INNER JOIN Raigarhproperty prop "
			+ "WHERE d.districtAccessBean_Sno = :district AND prop.khasra = :khasra)", nativeQuery = true)
	List<RaigarhParty2AccessBean> findByKhasraMethod(Long district, String khasra);
	...
	List<RaigarhParty2AccessBean> findByDateAndTehsilMethod(Long district, Long tehsil,String dateOfRegistration);
