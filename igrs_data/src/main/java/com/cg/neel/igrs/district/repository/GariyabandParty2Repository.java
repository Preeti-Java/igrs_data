...
@Repository
public interface GariyabandParty2Repository extends MappedTypeParty2Repository<GariyabandParty2AccessBean>{

	@Query(value = "SELECT * FROM Gariyabandparty2 p "
			+ "WHERE p.fileid IN (SELECT d.fileid FROM Gariyabanddeed d " + "INNER JOIN Gariyabandproperty prop "
	...
	List<GariyabandParty2AccessBean> findByMethod(Long year, Long tehsil, Long village,
			Long district);
	
	@Query(value = "SELECT * FROM Gariyabandparty2 p "
			+ "WHERE p.fileid IN (SELECT d.fileid FROM Gariyabanddeed d " + "INNER JOIN Gariyabandproperty prop "
			+ "WHERE d.districtAccessBean_Sno = :district AND prop.khasra = :khasra)", nativeQuery = true)
	List<GariyabandParty2AccessBean> findByKhasraMethod(Long district, String khasra);
	...
	List<GariyabandParty2AccessBean> findByDateAndTehsilMethod(Long district, Long tehsil,String dateOfRegistration);
