...
@Repository
public interface BalodParty2Repository extends MappedTypeParty2Repository<BalodParty2AccessBean>{
	
	@Query(value = "SELECT * FROM Balodparty2 p "
			+ "WHERE p.fileid IN (SELECT d.fileid FROM Baloddeed d " + "INNER JOIN Balodproperty prop "
	...
	List<BalodParty2AccessBean> findByMethod(Long year, Long tehsil, Long village,
			Long district);
	
	@Query(value = "SELECT * FROM Balodparty2 p "
			+ "WHERE p.fileid IN (SELECT d.fileid FROM Baloddeed d " + "INNER JOIN Balodproperty prop "
			+ "WHERE d.districtAccessBean_Sno = :district AND prop.khasra = :khasra)", nativeQuery = true)
	List<BalodParty2AccessBean> findByKhasraMethod(Long district, String khasra);
	...
	List<BalodParty2AccessBean> findByDateAndTehsilMethod(Long district, Long tehsil,String dateOfRegistration);
